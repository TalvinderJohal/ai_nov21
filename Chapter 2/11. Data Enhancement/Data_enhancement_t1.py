...
df_copy["t_av"] = (df_copy["t1"] + df_copy["t2"]) / 2
...
def data_enhancement(df_copy):
    ...
    for num in df_copy['t_av'].unique():
        coded_weather =  new_data[new_data['t_av'] == num]
        hum_std = coded_weather['hum'].std()
        wind_speed_std = coded_weather['wind_speed'].std()
        t1_std = coded_weather['t1'].std()
        t2_std = coded_weather['t2'].mean()

        for i in new_data[new_data["t_av"] == num].index:
            if np.random.randint(2) == 1:
                new_data['hum'].values[i] += hum_std/10
            else:
                new_data['hum'].values[i] -= hum_std/10
                
            if np.random.randint(2) == 1:
                new_data['wind_speed'].values[i] += wind_speed_std/10
            else:
                new_data['wind_speed'].values[i] -= wind_speed_std/10
                
            if np.random.randint(2) == 1:
                new_data['t1'].values[i] += t1_std/10
            else:
                new_data['t1'].values[i] -= t1_std/10
                
            if np.random.randint(2) == 1:
                new_data['t2'].values[i] += t2_std/10
            else:
                new_data['t2'].values[i] -= t2_std/10
    return new_data
